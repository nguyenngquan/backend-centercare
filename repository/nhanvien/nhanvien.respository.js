...
function HocvienRepository(dbContext) {
    ...
    function findHocvien(req, res, next) {
        ...
        if (req.params.hocvienId) {
            var parameters = [];

            parameters.push({ name: 'Id', type: TYPES.Int, val: req.params.hocvienId });

            var query = "select * from tbl_HocVien where Id = @Id"
    ...
    function getNhanviens(req, res) {
        ...
        if (req.query.salary) {
            ...
            dbContext.get("GetNhanvien", function (error, data) {
                return res.json(response(data, error));
            });
        }
    }
